...
{
	if(num_makers>MAX_NUM_B)return;
...
{
	double  var=  (px-X(0))*(px-X(0))+(py-X(1))*(py-X(1)); // x^2 +y^2
...
{
// H represents JHD in code this is the jacboian of H ONLY with respect to distance NOT including the angle (i.e JHD is (1,3) Matrix)
// As seen in JacobiH, this calculates the jacobian of H with respect to distance (first row), and angle (second row) we only care about the first row for HD
    double  var=  (px-X(0))*(px-X(0))+(py-X(1))*(py-X(1)); // x^2 +y^2
	double dist=sqrt(var);
	
    H<< -(px-X(0))/dist, -(py-X(1))/dist,   0; // Jacobian of H with respect to distance 
...
Robot::Robot(double _dt)
{
	...
	px=2.; py=6.; theta=.3; // Initial state
        steer=.01; speed=2.; // Control input
...
void Robot::LocalizeS(void)
{
	...
	S=Eigen::MatrixXd(1,1);     // Only considering distance therefore 1x1 matrix
	K=Eigen::MatrixXd(3,1); 
	I=Eigen::MatrixXd(3,3);
	I.setIdentity();
	error=Eigen::MatrixXd(1,1); //Only consider distance error therefore 1x1 matrix
	...
	for(i=0;i<num_satellites;i++){
		...
		double r_dist = satellites[i]->getDistance(px,py);
		error<<(r_dist-o_dist);
